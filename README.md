...
파이썬 스크립트가 네이버 뉴스 API로 대구경북 관련 기사를 모아서 중복·저관련성 기사를 걸러내고 중요도순으로 정리합니다. 깃허브 액션이 10분마다 자동으로 이 스크립트를 실행하고, 깃허브 페이지가 그 결과를 웹 대시보드로 보여줍니다. 컴퓨터를 꺼두어도 계속 갱신되고, 휴대폰에서도 접속할 수 있습니다.
...
기본적으로는 10분마다 자동 실행되지만, 바로 확인해보고 싶으시면 수동으로 한 번 돌릴 수 있습니다.
...
- **갱신 주기**: `.github/workflows/update.yml` 파일의 `cron: "*/10 * * * *"` 부분을 수정하면 됩니다. 숫자가 작을수록 더 자주 실행되지만, 그만큼 네이버 API 호출 횟수도 늘어납니다. 예를 들어 매시 정각에 한 번만 실행하려면 `"0 * * * *"`, 30분마다는 `"*/30 * * * *"` 입니다. 참고로 GitHub Actions는 스케줄 주기를 5분 단위 미만으로는 지원하지 않고, 실제로도 서버 부하에 따라 몇 분씩 밀려서 실행될 수 있습니다.
