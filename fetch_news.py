...
REGION_FALSE_POSITIVE_WORDS = [
    ...
    # "경북대"는 위에서 이미 지우지만, "대구경북첨단의료산업진흥재단"처럼
    # 기관명 안에 "대구"+"경북"이 붙어 있으면서 "경북" 뒤에 "대"가 안 오는
    # 경우(경북+첨단...)는 안 걸러졌습니다. 실제로 대구에 있는 "대구시-경북대
    # AI 신약개발 거점" 기사가 이 재단 이름 때문에 "경북" 기사로 잘못 분류된
    # 적이 있습니다. 이런 대구경북 공동 기관명은 지역 판단에 쓰지 않습니다.
    "대구경북첨단의료산업진흥재단",
]
...
EXCLUDE_KEYWORDS = [
    ...
    "팝업 행사", "프로모션으로",
    # 지자체 인사위원회 심의 결과·직급별 인사 명단 등 원문 그대로의 인사발령
    # 리스트 (뉴스라기보다 행정 공지). "정기인사"/"인사동정"은 이미 걸렀지만
    # "OO 인사위원회 심의 결과(날짜)"처럼 심의 절차 자체를 다루는 제목은
    # 놓치고 있었습니다.
    "인사위원회 심의 결과", "인사(5급 이하)",
    # 사설·칼럼·기고 계열 태그 중 "[사설]"처럼 짧은 고정 태그만 걸러내고
    # "[교육칼럼]"처럼 앞에 분야명이 붙은 칼럼 태그는 놓치고 있었습니다.
    # 대괄호형("[OO칼럼]")과 전각 대괄호형("【OO칼럼】") 둘 다 잡습니다.
    "칼럼]", "칼럼】",
    # 매장 오픈 등 홍보성 기사에서 "고객과의 접점을 확대"뿐 아니라 "고객과의
    # 접점을 넓힌다"처럼 다른 동사로 끝나는 경우도 있어 어간만 남겨 넓혔습니다.
    "고객과의 접점",
]
...
CATEGORY_RULES = [
    ("사건사고", ["사고", "화재", "방화", "붕괴", "추락", "폭발", "충돌", "전복", "실종", "익사",
                "교통사고", "산불", "침수", "붕괴사고"], 1.3),
    ...
    ("사회복지", ["교육", "복지", "의료", "병원", "학교", "환경", "폭염", "한파",
                "태풍", "호우", "지진", "재난"], 1.15),
    # "행사"/"관광"뿐 아니라 "음악회", "문화제", "축전"처럼 개최 안내 기사에
    # 자주 나오는데 기존 목록엔 없어서 "일반"(가중치 1.0)으로 새 나가던 표현을
    # 추가했습니다. 이런 정기 행사 개최 소식은 원래도 하드뉴스보다 중요도가
    # 낮아야 해서(가중치 0.85), 문화생활로 제대로 분류되는 게 맞습니다.
    ("문화생활", ["축제", "공연", "전시", "관광", "맛집", "행사", "박람회",
                "음악회", "문화제", "축전", "개최"], 0.85),
    ("스포츠", ["프로야구", "축구", "야구", "경기", "선수", "우승", "구단"], 0.9),
]

# 카테고리 키워드가 전혀 다른 뜻의 흔한 한국어 단어 안에 우연히 포함되는 경우를
# 막기 위한 목록입니다 (지역명 오탐을 막는 REGION_FALSE_POSITIVE_WORDS와 같은
# 원리). 예를 들어 "문화재단"에는 "화재"가, "경기도"에는 "경기"가, "사기진작"
# 에는 "사기"가 그대로 들어있어서, 카테고리 판단 전에 미리 지워야 합니다.
# (실제로 "대구 달성문화재단, '달성아리랑' 음원 제작"이라는 문화 소식이 이
# 문제 때문에 "사건사고"로 분류돼 중요도가 부풀려진 적이 있습니다.)
CATEGORY_FALSE_POSITIVE_WORDS = [
    "문화재",  # 사건사고 "화재" 오탐 (문화재/문화재단/문화재청 등)
    "이해충돌",  # 사건사고 "충돌" 오탐 (이해충돌방지법 등)
    "사고방식", "사고력", "인사고과", "역사고증", "창의적 사고", "비판적 사고",  # 사건사고 "사고" 오탐
    "사기진작", "사기 진작", "인사기록",  # 범죄사법 "사기" 오탐
    "경기도", "경기침체", "경기 침체", "경기부양", "경기 부양",
    "경기회복", "경기 회복", "경기둔화", "경기 둔화",  # 스포츠 "경기" 오탐
    "방화벽",  # 사건사고 "방화" 오탐 (IT 보안 용어 방화벽)
]


def strip_category_false_positives(text: str) -> str:
    for w in CATEGORY_FALSE_POSITIVE_WORDS:
        text = text.replace(w, "")
    return text
...
ROUTINE_ANNOUNCEMENT_KEYWORDS = [
    ...
    "글짓기대회", "글짓기 대회",
    # 3) 계절성·정기 반복 경보·통계 발표. "녹조"는 원래 여기 바로 넣었었는데,
    # 그러면 "낙동강 주민 소변서 녹조 독소 검출"처럼 실제로는 깊이 있는 취재가
    # 필요한 환경단체발 기사까지 단순 경보로 낮잡아 보게 돼서, "조류경보"처럼
    # 실제 정기 경보문에 쓰이는 형태로만 좁혔습니다.
    "조류경보", "조류 경보", "녹조경보", "녹조 경보", "폭염특보", "폭염 특보", "한파특보",
    ...
    # 7) 명절·연휴 등을 계기로 해마다 반복되는 치안·안전 캠페인 (그 자체로는
    # 지역 뉴스가 맞지만, 매번 비슷한 내용이 반복되는 정례 발표라 단신에 가깝
    # 습니다. 예: "광복절 폭주족 특별단속", "설 명절 특별교통대책" 등)
    "폭주족 특별단속", "명절 특별교통대책", "연휴 특별교통대책", "특별방역대책",
    # 8) 계절성 생활 안전 주의보 (사고 통계를 근거로 한 "조심하라"는 캠페인성
    # 기사로, 특정 사건 하나를 다루는 사건사고 기사와는 성격이 다릅니다)
    "사고 빈번", "물림 사고", "온열질환자", "물놀이 안전사고", "벌쏘임 사고",
    # 9) 가뭄·저수율 등 계절 반복 절수 캠페인·통계
    "물 아껴", "절수를 당부", "가뭄 지속", "저수율",
    # 10) 공모전·경진대회 수상작 발표 (2번 항목의 "선정" 문구 패턴에서 한 발
    # 더 나아가, 이미 진행된 대회의 결과 발표 자체도 포함합니다)
    "요리대회", "수상작품 선정", "수상작 선정", "수상작으로 선정",
    # 11) 소방·안전 분야에서 자주 열리는 "강의경연대회"류 발표대회 결과 (10번과
    # 같은 성격이지만 "선정"이 아니라 "최우수"/"우수상" 같은 등수 표현을 써서
    # 놓치고 있었습니다. 예: "이소라 고령 다산여성의용소방대원, 강의경연 '최우수'")
    "강의경연",
]
...
STOPWORDS = {
    ...
    "이라고", "라고", "라며", "이라며",
    # "위클리오늘"은 매일 각 시·군별로 나오는 요약 코너 이름이라, 중복 판단에
    # 쓰면 "[위클리오늘] 울진군 오늘(8. 13)"과 "[위클리오늘] 포항시 오늘(8. 13)"
    # 처럼 완전히 다른 지역의 요약 기사가 코너명·날짜만 겹쳐서 같은 사건으로
    # 잘못 묶이는 문제가 있었습니다 (제목 유사도 검사 중 발견).
    "위클리오늘",
}
...
def classify(article):
    text = strip_category_false_positives(article["title"] + " " + article["desc"])
    best = ("일반", 1.0)
